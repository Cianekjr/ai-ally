...
export class AuthResolver {
  ...
  async register(@Args('input') data: UserRegisterInput) {
    const { confirmationToken, confirmationDate } =
      this.authService.generateConfirmationToken();
  ...
  async login(
    @Args('input') user: UserLoginInput,
    ...
    if (!userData.isActive) {
      throw new Error(
        'Your email address is not confirmed. Complete the registration process first.',
      );
    }
  ...
  async forgotPassword(@Args('input') data: ForgotPasswordInput) {
    ...
    const { forgotPasswordToken, forgotPasswordDate } =
      this.authService.generateForgotPasswordToken();
