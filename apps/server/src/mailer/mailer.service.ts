...
export class MailerService {
  ...
  async sendMail({ templatePath, subject, to, locales }: { templatePath: string; subject: string; to: string; locales: NestedObject }) {
    try {
      if (!this.client) {
      ...
      await this.client.messages.create('sandbox3eb29995cbe0440398cb848be5a87c60.mailgun.org', {
  ...
  async sendRegisterMail({ to, locales }: { to: string; locales: { confirmationToken: string } }) {
    this.sendMail({
  ...
  async sendForgotPasswordMail({ to, locales }: { to: string; locales: { forgotPasswordToken: string } }) {
    this.sendMail({
